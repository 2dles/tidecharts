...
import { gearKeysFor } from "./gear-keys";
...
// Gear keys prefer the store-generated BY_SPECIES map (see gear-keys.ts);
// the hand-written lists above/in species-fl.ts are the fallback.
export const SPECIES: Record<string, Species> = Object.fromEntries(
  Object.entries({ ...CA_SPECIES, ...FL_SPECIES }).map(([k, s]) => [
    k,
    { ...s, gearKeys: gearKeysFor(k, s.gearKeys) },
  ]),
);
